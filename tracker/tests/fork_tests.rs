...
#[test]
fn reorg_to_shortest_chain() {
    let storage = MemoryIndexStorage::new();

    let _tempdir = TempDir::new("/tmp/test_reorg_to_shortest_chain/".to_string());
    let dir1 = "/tmp/test_reorg_to_shortest_chain/node1/";
    let dir2 = "/tmp/test_reorg_to_shortest_chain/node2/";

    const OFFSET_N1: u32 = 5;
    const OFFSET_N2: u32 = 6;
    const NODE2_PORT: u32 = 18444 + OFFSET_N2;
    let node2_addr = format!("localhost:{}", NODE2_PORT);
    ...
    // Mine block with Bag2_1 on node 1.
    let (last_block_chain_1, prf2_1) = {
        // NB: send SATOSHIES_TO_SEND*100 to make the chain has more weight.
        let bid = client1
            .send_mint_transaction(SATOSHIES_TO_SEND * 100, &BAG2_1)
            .unwrap();
        let block = generate_block(&client1, &address1, &bid.outpoint.txid);
        (block, BagProof::new(block, bid))
    };
    ...
    // Track chain on node2.
    let bags = BagMemoryStorage::new();
    let mut index = Index::new(client2, storage, bags, Some(HEIGHT_BEFORE_FORK - 1));

    // Tracker has no access to the chain #1, so it cannot prove bag 2_1 now.
    index.add_bid(prf1_12).unwrap();
    index.add_bag(prf2_1.bid_tx.bag_id).unwrap();
    index.add_bid(prf2_2).unwrap();
    index.add_bid(prf3_2).unwrap();

    // Check that node2 contains 3 bags on chain #2.
    {
        assert_eq!(*index.current_height(), HEIGHT_CHAIN2);
    ...
    // Reconnect node1 with node2.
    let client2 = index.btc_client();
    add_node_client(&client1, &node2_addr);
    assert_eq!(client1.get_network_info().unwrap().connections, 1);
    wait!(client2.get_blockchain_info().unwrap().blocks == HEIGHT_CHAIN1);

    // Tracker must find reorg there.
    index.check_reorgs();

    // Check that reorg happened and chain #1 is main now.
    {
        assert_eq!(*index.current_height(), HEIGHT_CHAIN1);
